...
const empty = { name: "", role_title: "", committee: "", photo_url: "", bio: "", sort_order: 0 };

const EBTab = () => {
  const [rows, setRows] = useState<EBMember[]>([]);
  ...
  const load = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("executive_board").select("*").order("sort_order").order("name");
    if (error) toast.error(error.message);
    else setRows((data ?? []) as EBMember[]);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);

  const startNew = () => { setEditing(null); setForm(empty); };
  const startEdit = (m: EBMember) => {
    setEditing(m);
    setForm({
      name: m.name, role_title: m.role_title ?? "", committee: m.committee ?? "",
      photo_url: m.photo_url ?? "", bio: m.bio ?? "", sort_order: m.sort_order,
    });
  };
  ...
  const save = async () => {
    ...
    const { error } = editing
      ? await supabase.from("executive_board").update(payload).eq("id", editing.id)
      : await supabase.from("executive_board").insert(payload);
    if (error) return toast.error(error.message);
    toast.success(editing ? "EB Member updated" : "EB Member added");
    startNew(); load();
  };

  const remove = async (id: string) => {
    if (!confirm("Remove this EB member?")) return;
    const { error } = await supabase.from("executive_board").delete().eq("id", id);
    if (error) return toast.error(error.message);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-white text-xl font-bold">Manage Executive Board</h2>
      ...
      <div className="glass rounded-2xl p-4 grid md:grid-cols-2 gap-4">
        ...
        <Field label="Role / Title">
          <Input value={form.role_title} onChange={(e) => setForm({ ...form, role_title: e.target.value })} className={inputCls} placeholder="Chairperson" />
        </Field>
        <Field label="Committee">
          <Input value={form.committee} onChange={(e) => setForm({ ...form, committee: e.target.value })} className={inputCls} placeholder="UNSC" />
        </Field>
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {rows.length === 0 ? (
            <p className="text-white/60 text-sm col-span-full">No EB members yet.</p>
          ) : rows.map((m) => (
            <div key={m.id} className="glass rounded-xl p-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/60 text-xs truncate">{m.role_title ?? ""}{m.committee ? ` · ${m.committee}` : ""}</p>
              </div>
